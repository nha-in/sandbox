"""The review team's console: the support queue, the vendor register, and events.

Every view here inherits StaffConsoleMixin, and StaffConsoleMixin exists to make the
...
guesses a URL under /staff/ is looking at every other vendor's tickets — so it is
...
from .forms import staff_members
...
class StaffConsoleMixin(StaffConsoleMixin):
...
class TicketQueueView(StaffConsoleMixin, ListView):
    ...
    template_name = "staff/queue.html"
    ...
    partial_template_name = "staff/partials/queue_results.html"
    ...
    def filter_form(self) -> TicketFilterForm:
        ...
        With no status in the query string the queue shows the review team's own
    ...
    def get_queryset(self):
        ...
        if status == Status.OPEN:
            ...
            tickets = tickets.awaiting_staff()
        elif status:
    ...
    def showing_default_queue(self) -> bool:
        ...
        This is what the empty state turns on. "Nothing is waiting on the review
        team" is only true of `awaiting_staff()` with no other filter applied —
...
class TicketDetailView(StaffConsoleMixin, View):
    """Work one ticket: the conversation, the reply box and the control rail."""

    def get(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ticket = get_ticket(kwargs["reference"])
        return render(request, "staff/ticket_detail.html", workspace_context(ticket))


class TicketActionView(StaffConsoleMixin, View):
    ...
    def swap(self, request: HttpRequest, ticket: Ticket, **forms) -> HttpResponse:
        if request.htmx:
            return render(
                request,
                "staff/partials/ticket_workspace_swap.html",
                workspace_context(ticket, **forms),
            )
        return redirect("staff:ticket", reference=ticket.reference)
    ...
    def reshow(self, request: HttpRequest, ticket: Ticket, **forms) -> HttpResponse:
        ...
        if request.htmx:
            return render(request, "staff/partials/ticket_workspace_swap.html", context)
...
class OrganisationListView(StaffConsoleMixin, ListView):
    ...
    template_name = "staff/organisation_list.html"
    # The page includes this fragment; a filter change swaps the same file back
    # in, so a filtered list and a reloaded page are the same markup.
    partial_template_name = "staff/partials/organisation_results.html"
...
class OrganisationDetailView(StaffConsoleMixin, View):
    ...
    def get(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        return render(
            request,
            "staff/organisation_detail.html",
            register_context(organisation),
        )


class OrganisationVerificationView(StaffConsoleMixin, View):
    ...
    def post(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        return redirect("staff:organisation", slug=organisation.slug)
    ...
    def render_register(
        ...
        **forms,
        ...
        if request.htmx:
            return render(
                request,
                "staff/partials/organisation_register_swap.html",
                context,
            )
        return render(request, "staff/organisation_detail.html", context)


# ── Events ─────────────────────────────────────────────────────────────────


class EventListView(StaffConsoleMixin, ListView):
...
class EventFormMixin(StaffConsoleMixin):
    model = Event
    ...
    success_url = reverse_lazy("staff:events")
    nav_section = "events"
...
class EventPublishToggleView(StaffConsoleMixin, View):
    ...
    def post(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        if request.htmx:
            ...
            return render(request, "staff/partials/event_swap.html", {"event": event})
