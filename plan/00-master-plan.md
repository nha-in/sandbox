...
5. **Should a `DRAFT` be allowed to hold half-finished work?** [A3](v0-tickets/A3-applications-model.md)'s `create_draft` and `update_draft` both call `validate_payload`, so an `Application` row is always complete and valid — an empty draft cannot be written. [C4](v0-tickets/C4-enrollment-wizard.md) therefore holds the product choice in the session for one step and creates the row only once the payload form passes, which means the long details step is **not** resumable: close the browser mid-form and those answers are gone. That is the exact pain the legacy four-page React wizard was criticised for. The alternative is to let drafts be partial and move validation onto the `SUBMIT` transition, where it is the guard that actually matters — at the cost of every reader of `payload` having to tolerate missing keys. _(Internal decision, no NHA input needed. Shapes [A3](v0-tickets/A3-applications-model.md) + [C4](v0-tickets/C4-enrollment-wizard.md); worth settling before [C6](v0-tickets/C6-integrator-dashboard.md) and [C8](v0-tickets/C8-milestone-exit-forms.md) build more forms on the same assumption.)_
6. **Who may create an organisation, and does it need approval?** Nobody owned this: sign-up creates a `User` with no tenant, [A2](v0-tickets/A2-users-organisations-org-scoping.md) deferred only _team invites_, and no service, view or form anywhere created an `Organisation` — so a self-registered integrator was a permanent dead end and V0.2's enrol→approve criterion was only reachable through the seed. Legacy had no such gap because sign-up _was_ enrolment: `SdLogin` carried the organisation fields, so there was no separate organisation to create. **Decided:** self-service creation, unlimited for now — any verified non-staff user may create organisations and becomes `OWNER` of each; every one stays `PENDING` until staff verify it, so creating one grants no standing. **Deferred, not resolved:** there is no per-user cap and no rate limit, so an email-verified account can create unbounded `PENDING` organisations — an abuse vector that needs closing before any public deployment. _(Shapes [A2](v0-tickets/A2-users-organisations-org-scoping.md)/[C6](v0-tickets/C6-integrator-dashboard.md) and the P2 team-invites work. See also questions 8 and 9.)_
7. **What does a reviewer see when the organisation profile changes under them?** Organisation facts are deliberately _not_ copied into the application payload — the application references its tenant and nothing else — so a reviewer always sees the profile as it is _now_, not as it was at submission. Two consequences neither of which is handled: an owner can edit the registered address or GSTIN while an application is under review and silently change what is being assessed; and an approval is not reproducible, because nothing records the facts that were on screen when it was granted. Compounding this, `update_organisation_profile` emits no audit event (only `workflow` and `declarations` call `audit.emit`), and `verification_state` is never written by application code, so a `VERIFIED` organisation can be rewritten without re-verification. Options: audit every profile change; freeze the profile while any application is `SUBMITTED`; or record a submit-time snapshot as evidence. _(Deliberately deferred. Shapes [A2](v0-tickets/A2-users-organisations-org-scoping.md)/[C5](v0-tickets/C5-console-review-queue.md); matters most before a real approval carries legal weight.)_
8. **Does NHA accept self-service organisation creation, or must integrators be vetted before they get a tenant?** v0 lets any contact-verified, non-staff user create an organisation and own it immediately (question 6). That is the right default for a self-service sandbox and matches the legacy portal's open sign-up, but legacy conflated the two: registering _was_ enrolling, so the question never had to be asked. v2 separates them, which makes "who is allowed to exist as a tenant" a real decision. The three shapes are self-service (today), self-service with a staff gate before the first application may be submitted, or staff-provisioned only. The choice determines whether the abuse vector in question 6 is a bug to fix or a non-issue. _(Needs NHA. Blocks nothing today; shapes [A2](v0-tickets/A2-users-organisations-org-scoping.md) and the P2 team-invites work, and should be settled before any public or pre-production deployment.)_
9. **Where is an organisation actually verified, and what does verification gate?** `Organisation.verification_state` (`PENDING`/`VERIFIED`, plus `verified_by`/`verified_at`) exists on the model and is written by **nothing** — no service, no view, only the Django admin. No screen shows it, no transition consults it, and no application is blocked by it, so today `PENDING` is a column with no meaning: an unverified organisation can enrol, be approved and be provisioned exactly like a verified one. Either it needs a reviewer surface and a guard (most naturally in [C5](v0-tickets/C5-console-review-queue.md)'s console, gating `SUBMIT` or `APPROVE`), or it is premature and the columns should be dropped until the workflow that needs them is designed. Legacy is no guide here: it had no organisation entity to verify. _(Internal design decision, informed by question 8. Shapes [A2](v0-tickets/A2-users-organisations-org-scoping.md)/[C5](v0-tickets/C5-console-review-queue.md); leaving a dead column in the schema is the worst of the three options.)_
10. **Should a reviewer be able to send an application back, or only comment?** Today they can only comment: `review_application` records a review row, every transition needs its own permission, and the reviewer is granted none — so a reviewer who wants changes must find an admin to press the button. This was not decided, it drifted: [C5](v0-tickets/C5-console-review-queue.md) shipped with `SEND_BACK` gated on `review_application`, [A5](v0-tickets/A5-workflow-state-machine.md) later split `send_back_application` out as its own permission, nothing granted it to reviewers, and the test was updated to match the new behaviour rather than the documented intent. The argument for granting it: asking for corrections is the commonest and least consequential reviewer action, and routing every "please clarify" through an admin makes the admin the bottleneck for work that carries no decision. The argument against: any state change on a government application is an exercise of authority and should sit with the people accountable for the outcome. Note this interacts with the separation-of-duties gap — nothing stops one person reviewing and then deciding on their own review. _(Internal decision, no NHA input needed. One line in the seed either way; settle it before the reviewer role is described to NHA.)_
11. **How do the 16 registration solution types map to the 5 DHIS ones?** v0 maps by name — the five values that appear in both vocabularies (`HMIS`, `LMIS`, `TELEMEDICINE`, `PHARMACY`, `HEALTH_LOCKER`) map to themselves, and the other eleven grant nothing. Legacy does something materially wider, verified in `SdLoginServiceImpl.mapSolutionTypesToDhis()`: `HMIS`, `Clinic HMIS` and `Govt HMIS` each **expand** to `{HMIS, LMIS, Pharmacy}`, so one selection grants three, and every other value is preserved verbatim (`EUA` becomes an inert `"EUA"` in the DHIS column). The result is written by `appendToDhisSolutionTypeWithUniqueness()`, an **append-only ratchet** fed from both registration edits and exit decisions — narrowing your solution types never narrows the grant. Two legacy defects fall out of this and are already fixed by construction in [09-redesign](09-redesign.md): `WorkflowServiceImpl.changeExitStatus()` runs the append on the **`REJECTED`** branch too (a rejected exit grants DHIS types; partly masked because `integration_level` is only updated on the approved branch), and the ratchet means a grant outlives the claim that earned it — v2 unions over approved exits only, at the claims as they stood when decided. The open part is the expansion rule itself: porting it lets an HMIS-only integrator register a pharmacy solution on DHIS without ever claiming to have built one, but not porting it narrows who reaches DHIS relative to production today. v0 deliberately takes the narrow reading because the admin's approved-solution-types checkboxes at exit are an explicit, auditable version of the same widening, bounded by the registration ceiling. _(Needs NHA to confirm the expansion was intended. Shapes `dhis_enabled`/`is_compliant` only — gates nothing in v0, since DHIS registration is a v1 surface.)_
