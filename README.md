...
- **Styling is a small component layer**: classes live in `theme/static_src/src/ui.css`, colour comes from semantic tokens (`bg-primary`, `text-muted-foreground`, `border-border`) and never a raw hex. New components land there, not as utility soup in templates.
- **Forms render through `{% ui_field %}`** (`theme/templatetags/ui.py`), which is also how allauth's own forms get styled. There is no crispy.
